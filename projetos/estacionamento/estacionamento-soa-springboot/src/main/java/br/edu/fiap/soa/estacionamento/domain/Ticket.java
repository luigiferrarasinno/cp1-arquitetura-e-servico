...
public class Ticket {
    ...
    @JoinColumn(name = "reserva_id", foreignKey = @ForeignKey(name = "fk_ticket_reserva"))
    private Reserva reserva;
    
    @PrePersist public void prePersist(){ 
        if(entrada==null) entrada=LocalDateTime.now(); 
        if(status==null) status=TicketStatus.ABERTO;
        if(tipoTarifa==null) tipoTarifa=TipoTarifa.HORARIA;
    }
}
